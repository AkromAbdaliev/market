...
std::vector<std::pair<Product,ProductCategory>> Category::getProduct(const DbClientPtr &clientPtr) const {
    static const std::string sql = "select * from product,product_category where product_category.category_id = $1 and product_category.product_id = product.id";
    Result r(nullptr);
    {
    ...
    std::vector<std::pair<Product,ProductCategory>> ret;
    ret.reserve(r.size());
    for (auto const &row : r)
    {
        ret.emplace_back(std::pair<Product,ProductCategory>(
            Product(row),ProductCategory(row,Product::getColumnNumber())));
    }
    return ret;
}

void Category::getProduct(const DbClientPtr &clientPtr,
                          const std::function<void(std::vector<std::pair<Product,ProductCategory>>)> &rcb,
